...
<form class="row g-3" method="post" action="/formulir/update/<?= $formulir['id_formulir']; ?>" id="form" enctype="multipart/form-data">

    <div class="col-md-6">
        <label for="nama_pendaftar" class="form-label">Nama Sesuai Ijazah <span class="text-danger">*</span></label>
        <input type="text" class="form-control <?= validation_show_error('nama_pendaftar') ? 'is-invalid' : ''; ?>" id="nama_pendaftar" name="nama_pendaftar" required value="<?= $formulir['nama_pendaftar']; ?>">
    ...
    <div class="col-md-6">
        ...
        <input type="text" class="form-control <?= validation_show_error('telpon_pendaftar') ? 'is-invalid' : ''; ?>" id="telpon_pendaftar" name="telpon_pendaftar" required value="<?= $formulir['telpon_pendaftar']; ?>">
        <div class="invalid-feedback">
    ...
    <div class="col-md-6">
        ...
        <select class="form-select <?= validation_show_error('pilihan_pertama') ? 'is-invalid' : ''; ?>" id="pilihan_pertama" name="pilihan_pertama" required>
            ...
            <?php foreach ($prodi as $row) : ?>
                <option value="<?= $row['id_prodi']; ?>" <?= $formulir['pilihan_pertama'] == $row['id_prodi'] ? 'selected' : ''; ?>><?= $row['prodi']; ?></option>
            <?php endforeach ?>
        </select>
        <div class="invalid-feedback">
    ...
    <div class="col-md-6">
        ...
        <select class="form-select <?= validation_show_error('pilihan_kedua') ? 'is-invalid' : ''; ?>" id="pilihan_kedua" name="pilihan_kedua" required>
            <option value="">- Pilih Prodi -</option>
            <?php foreach ($prodi as $row) : ?>
                <option value="<?= $row['id_prodi']; ?>" <?= $formulir['pilihan_kedua'] == $row['id_prodi'] ? 'selected' : ''; ?>><?= $row['prodi']; ?></option>
            <?php endforeach ?>
        </select>
        <div class="invalid-feedback">
    ...
    <div class="col-md-4">
        ...
        <div class="text-center">
            <img src="/assets/img/<?= $formulir['foto']; ?>" alt="" width="60%" class="mt-3 rounded" id="preview">
        </div>
    </div>
    ...
    <div class="col-md-4">
        ...
        <div class="text-center">
            <img src="/assets/img/<?= $formulir['ijazah']; ?>" alt="" width="60%" class="mt-3 rounded" id="preview-2">
        </div>
    </div>
    ...
    <div class="col-md-4">
        ...
        <div class="text-center">
            <img src="/assets/img/<?= $formulir['pembayaran_pendaftaran']; ?>" alt="" width="60%" class="mt-3 rounded" id="preview-3">
        </div>
    </div>

    <input type="hidden" name="foto_lama" value="<?= $formulir['foto']; ?>">
    <input type="hidden" name="ijazah_lama" value="<?= $formulir['ijazah']; ?>">
    <input type="hidden" name="pembayaran_pendaftaran_lama" value="<?= $formulir['pembayaran_pendaftaran']; ?>">
